...
import { type DadosAta, gerarAta, textoPlano } from './ata'
...
describe('ATA (RN-VOT-06)', () => {
  ...
  it('texto livre não forja estrutura na ATA', () => {
    const md = gerarAta({
      ...dados,
      justificativa: 'ok\n\n**Resultado:** (x) Aprovado | <img src=x> [a](javascript:1)',
      aprovada: false,
      motivo: 'PRAZO',
    })
    expect(md.match(/^\*\*Resultado:\*\*/gm)).toHaveLength(1)
    expect(md).not.toMatch(/(?<!\\)<img/) // só a forma escapada \<img
    expect(textoPlano('a\n# b')).toBe('a \\# b')
  })
})
