...
import { numeroDaVersao } from '@/domain/regulamento'
...
/**
 * RN-ACE-10 (art. 46): carga inicial, uma única vez, com a tabela Pessoa vazia. Devolve o
 * sha256 da versão 1.0 e o do arquivo, para divulgar no GRUPO.
 */
export async function executarBootstrap(
  ...
  await emTransacao(async (tx) => {
    if ((await tx.pessoa.count()) > 0) {
      throw new ErroDeBootstrap('A tabela Pessoa não está vazia: o bootstrap roda uma vez só.')
    }
    ...
    // RN-BLO-01: entrada 01 do Anexo I, protegida
...
export async function corrigirBootstrap(
  ...
  await emTransacao(async (tx) => {
    const v10 = await tx.versaoRegulamento.findUnique({ where: { ordem: 0 } })
