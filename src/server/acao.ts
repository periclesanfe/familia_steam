...
export type ContextoAcao = Contexto & {
  ator: { tipo: 'MEMBRO'; pessoaId: string }
  sessaoId: string
  perfil: PerfilAtual
}
...
export function acao<S extends z.ZodType, D>(
  ...
  opcoes: { perfis?: readonly Perfil[] } = {},
) {
  const perfis = opcoes.perfis ?? ['MEMBRO'] // nega por padrão (14 SEG-01)
  const executar = async (
    ...
    if (!perfil || !perfis.includes(perfil.perfil)) {
      log.aviso('acesso.negado', { pessoaId: sessao.pessoaId, perfil: perfil?.perfil ?? null })
      return falha('SEM_PERMISSAO', fd) as EstadoAcao<D>
    }
