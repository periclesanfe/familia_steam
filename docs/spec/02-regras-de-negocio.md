...
- **RN-REG-02 — Antes da vigência.** Só estão disponíveis login, onboarding, leitura e edição do texto (RN-REG-08) e adesão. Não se cria rodada, obrigação nem votação.
...
- **RN-REG-08 — Rascunho editável e pedido de alteração** (arts. 42 e 46; M10). **Antes da vigência**, qualquer membro aberto da família (`PENDENTE` ou `MEMBRO`) edita o texto e os parâmetros da 1.0 em `/regulamento/editar`, com um resumo obrigatório. A edição troca o `sha256` da 1.0, então as adesões já feitas deixam de ser válidas (`adesaoValida`) e todos assinam de novo o texto final: o acordo só entra em vigor quando todos assinam o **mesmo** texto (RN-REG-01). Cada edição fica na auditoria (`regulamento.rascunho`, com o texto antes e depois), de onde sai o histórico com diff em boas-vindas. Lock `'regulamento'` (o mesmo da assinatura). **Depois da vigência** a edição direta é recusada (`REGULAMENTO_EM_VIGOR`): o membro propõe a alteração no mesmo editor, com justificativa, e ela vira votação `ALTERACAO_REGULAMENTO` (RN-REG-03/04; uma aberta por vez). Texto sem mudança é recusado (`SEM_ALTERACAO`).
