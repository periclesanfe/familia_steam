...
- **Append-only no privilégio**, além do trigger: `REVOKE UPDATE, DELETE, TRUNCATE` de `app_rw` em `sorteio`, `voto`, `ata`, `adesao` e `evento_auditoria`, e `REVOKE DELETE, TRUNCATE` em `obrigacao`, `pagamento`, `aviso_compra` e `aquisicao` (espelhando `bloquear_mutacao`/`bloquear_delete`). `TRUNCATE` não dispara trigger de linha, e é o `REVOKE TRUNCATE` que o bloqueia para o `app_rw`. Um trigger `BEFORE TRUNCATE` não acrescentaria nada: o dono pode desabilitá-lo. Implementado em `20260925000000_privilegios` e `docker/initdb/01-papeis.sh`.
